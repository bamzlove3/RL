...
function checkAbuseSub( elForm )
{
	...
	$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', params).done( function() {
		ShowAlertDialog( 'Спасибо!', 'Благодарим вас за сообщение об оскорбительном контенте. Вы помогаете поддерживать чистоту и дружеские отношения в сообществе Steam.' );
	}).fail( function() {
		ShowAlertDialog( 'Сообщить о нарушении', 'Произошла ошибка при попытке сообщить о нарушении. Пожалуйста, попробуйте позже.' );
	});
	return false;
}
...
function ShowAbuseDialog()
{
	...
	if ( g_AbuseModalContents )
	{
		var Modal = ShowDialog( 'Сообщить о нарушении', g_AbuseModalContents );
	}
}
...
function ShareContentToUserStatus( text, urlToShare, appID, posturl )
{
	...
	new Ajax.Request( posturl, {
		...
		onSuccess: function(transport) {
			...
			ShowAlertDialog( 'Поделиться', 'Обновление статуса было записано в вашей ленте активности.' );
		},
		onFailure: function(transport) {
			ShowAlertDialog( 'Поделиться', 'Во время обновления статуса произошла ошибка. Пожалуйста, повторите попытку позже.' );
		}
	});
}
