...
function checkAbuseSub( elForm )
{
	...
	$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', params).done( function() {
		ShowAlertDialog( 'Dziękujemy!', 'Dziękujemy za zgłoszenie obraźliwych treści i dbałość o dobre imię Społeczności Steam.' );
	}).fail( function() {
...
function ChangeLanguage( strTargetLanguage, bStayOnPage )
{
	...
	$J.post( 'https://steamcommunity.com/actions/SetLanguage/', {language: strTargetLanguage, sessionid: g_sessionID })
		.done( function() {
			...
			Modal.Dismiss();
			ShowAlertDialog( 'Zmień język', 'Wystąpił błąd z komunikacją z serwerami Steam. Spróbuj ponownie później.' );
		});
}
...
var CCommentThread = Class.create( {
	...
	initialize: function( type, name, rgCommentData, url, nQuoteBoxHeight )
	{
		...
		this.m_$SubscribeCheckbox.click( function() {
			...
			if ( _this.m_bSubscribed )
				_this.Unsubscribe( function() {},  function() {ShowAlertDialog('Zasubskrybuj wątek', 'Wystąpił błąd przy aktualizacji subskrypcji. Spróbuj ponownie później.');});
			else
				_this.Subscribe( function() {},  function() {ShowAlertDialog('Zasubskrybuj wątek', 'Wystąpił błąd przy aktualizacji subskrypcji. Spróbuj ponownie później.');});
		});
	...
	DisplayError: function( elError, transport )
	{
		...
		if ( transport.responseJSON && transport.responseJSON.error )
			...
			strMessage += 'Wystąpił błąd z komunikacją z siecią. Spróbuj ponownie później.';
	...
	BindSubscribeButtons: function( elBtnSubscribe, elBtnUnsubscribe )
	{
		...
		$J(elBtnSubscribe).click( function() {
			_this.Subscribe( function() {
				ShowAlertDialog('Zasubskrybuj wątek', 'Otrzymasz powiadomienie o komentarzu, kiedy ktoś odpowie w tym wątku.');
			}, function() {
				ShowAlertDialog('Zasubskrybuj wątek', 'Wystąpił błąd przy aktualizacji subskrypcji. Spróbuj ponownie później.');
			});
		});

		$J(elBtnUnsubscribe).click( function() {
			_this.Unsubscribe( function() {
				ShowAlertDialog('Anuluj subskrypcję tego wątku', 'Nie otrzymasz więcej powiadomień o nowym komentarzu w tym wątku.');
			}, function() {
				ShowAlertDialog('Anuluj subskrypcję tego wątku', 'Wystąpił błąd przy aktualizacji subskrypcji. Spróbuj ponownie później.');
			});
		});
...
function ShowSharePopup( url, baseSocialShareURL )
{
	...
	gSharePopup = ShowDialog( 'Podziel się', $( 'SharePopup' ) );
...
function ShareOnSteam()
{
	...
	gShareOnSteamDialog = _BuildDialog(  'Podziel się', $( 'ShareOnSteamDialog' ), [], fnCancel, null );
...
function ShareContentToUserStatus( text, urlToShare, appID, posturl )
{
	...
	new Ajax.Request( posturl, {
		...
		onSuccess: function(transport) {
			...
			ShowAlertDialog( 'Podziel się', 'Aktualizacja statusu została umieszczona w twojej Aktywności Znajomych.' );
		},
		onFailure: function(transport) {
			ShowAlertDialog( 'Podziel się', 'Wystąpił problem z aktualizacją statusu. Spróbuj ponownie później.' );
		}
	});
}
