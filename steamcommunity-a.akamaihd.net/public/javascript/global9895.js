...
function checkAbuseSub( elForm )
{
	...
	{
		alert( 'Моля посочете причина за докладването на обида' );
		return false;
	}
	...
	$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', params).done( function() {
		ShowAlertDialog( 'Благодарим Ви!', 'Благодарим Ви за докладването на обидно съдържание и спомагането да опазим Steam общността чиста и приятелски настроена.' );
	}).fail( function() {
...
function AlertNonSteamSite( elem )
{
	...
	if ( hostname )
	{
		...
		return confirm( 'Забележка: Адресът, на който кликнахте, не е официален Steam уебсайт.\n\n'
						...
						+ 'Сигурни ли сте, че искате да посетите тази страница? Натиснете Добре, за да продължите на собствена отговорност.\n' );
	}
...
{
	var Modal = ShowBlockingWaitDialog( 'Смяна на езикa', '' );
	$J.post( 'https://steamcommunity.com/actions/SetLanguage/', {language: strTargetLanguage, sessionid: g_sessionID })
		.done( function() {
			...
			Modal.Dismiss();
			ShowAlertDialog( 'Смяна на езикa', 'Имаше проблем при комуникацията със Steam сървърите. Моля, опитайте отново по-късно.' );
		});
}
	...
	BindSubscribeButtons: function( elBtnSubscribe, elBtnUnsubscribe )
	{
		...
		$J(elBtnUnsubscribe).click( function() {
			_this.Unsubscribe( function() {
				ShowAlertDialog('Премахване на абонамента', 'Вече няма да получавате известия за коментари в тази тема.');
			}, function() {
				ShowAlertDialog('Премахване на абонамента', 'Имаше проблем при обновяването на абонамента Ви. Моля, опитайте отново по-късно.');
			});
		});
...
function ShareContentToUserStatus( text, urlToShare, appID, posturl )
{
	...
	new Ajax.Request( posturl, {
		...
		onFailure: function(transport) {
			ShowAlertDialog( 'Споделяне', 'Имаше проблем при споделянето на статус обновлението. Моля, опитайте по-късно.' );
		}
	});
}
