...
function checkAbuseSub( elForm )
{
	...
	$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', params).done( function() {
		...
		ShowAlertDialog( 'Rapporter regelbrudd', 'Det oppstod en feil ved lagring av rapporten din. Vennligst prøv igjen senere.' );
	});
	return false;
}
...
function ShowAbuseDialog()
{
	...
	if ( g_AbuseModalContents )
	{
		var Modal = ShowDialog( 'Rapporter regelbrudd', g_AbuseModalContents );
	}
}
...
function ChangeLanguage( strTargetLanguage, bStayOnPage )
{
	...
	$J.post( 'https://steamcommunity.com/actions/SetLanguage/', {language: strTargetLanguage, sessionid: g_sessionID })
		.done( function() {
			...
			Modal.Dismiss();
			ShowAlertDialog( 'Bytt språk', 'Et problem oppstod under kommunikasjon med Steams tjenere. Prøv igjen senere.' );
		});
}
	...
	BindSubscribeButtons: function( elBtnSubscribe, elBtnUnsubscribe )
	{
		...
		$J(elBtnUnsubscribe).click( function() {
			_this.Unsubscribe( function() {
				...
				ShowAlertDialog('Unsubscribe from thread', 'Det oppstod problemer med oppdatering av abonnementet ditt. Prøv igjen senere.');
			});
		});
...
function ShareContentToUserStatus( text, urlToShare, appID, posturl )
{
	...
	new Ajax.Request( posturl, {
		...
		onFailure: function(transport) {
			ShowAlertDialog( 'Del', 'Det oppstod et problem med å dele denne statusoppdateringen. Vennligst prøv igjen senere.' );
		}
	});
}
