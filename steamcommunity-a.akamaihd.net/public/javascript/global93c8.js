...
function checkAbuseSub( elForm )
{
	...
	{
		alert( 'Por favor, selecione uma razão para denunciar abuso' );
		return false;
	}
	...
	$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', params).done( function() {
		ShowAlertDialog( 'Obrigado!', 'Obrigado por denunciar conteúdo ofensivo e ajudar a manter a Comunidade Steam limpa e amigável.' );
	}).fail( function() {
		ShowAlertDialog( 'Denunciar violação', 'Houve um problema ao salvar a sua denúncia. Por favor, tente novamente mais tarde.' );
	});
	return false;
}
...
function AlertNonSteamSite( elem )
{
	...
	if ( hostname )
	{
		...
		return confirm( 'Aviso: o link que você clicou não é um site oficial do Steam.\n\n'
						...
						+ 'Se esse site pedir pelo seu nome de usuário ou senha, não insira tais informações. Você pode perder a sua conta Steam e todos os seus jogos!\n'
...
function ChangeLanguage( strTargetLanguage, bStayOnPage )
{
	...
	$J.post( 'https://steamcommunity.com/actions/SetLanguage/', {language: strTargetLanguage, sessionid: g_sessionID })
		.done( function() {
			...
			Modal.Dismiss();
			ShowAlertDialog( 'Alterar idioma', 'Houve um problema ao comunicar-se com os servidores Steam. Por favor, tente novamente mais tarde.' );
		});
}
...
var CCommentThread = Class.create( {
	...
	initialize: function( type, name, rgCommentData, url, nQuoteBoxHeight )
	{
		...
		this.m_$SubscribeCheckbox.click( function() {
			...
			if ( _this.m_bSubscribed )
				_this.Unsubscribe( function() {},  function() {ShowAlertDialog('Inscrever-se no tópico', 'Houve um problema ao atualizar a sua inscrição. Por favor, tente novamente mais tarde.');});
			else
				_this.Subscribe( function() {},  function() {ShowAlertDialog('Inscrever-se no tópico', 'Houve um problema ao atualizar a sua inscrição. Por favor, tente novamente mais tarde.');});
		});
	...
	OnSubmit: function()
	{
		...
		{
			ShowAlertDialog( '', 'Por favor, insira um comentário para publicar.' );
			return;
		}
	...
	DisplayError: function( elError, transport )
	{
		...
		if ( transport.responseJSON && transport.responseJSON.error )
			...
			strMessage += 'Houve um erro ao comunicar-se com a rede. Por favor, tente novamente mais tarde.';
	...
	BindSubscribeButtons: function( elBtnSubscribe, elBtnUnsubscribe )
	{
		...
		$J(elBtnSubscribe).click( function() {
			_this.Subscribe( function() {
				...
				ShowAlertDialog('Inscrever-se no tópico', 'Houve um problema ao atualizar a sua inscrição. Por favor, tente novamente mais tarde.');
			});
		});
		...
		$J(elBtnUnsubscribe).click( function() {
			_this.Unsubscribe( function() {
				...
				ShowAlertDialog('Cancelar inscrição', 'Houve um problema ao atualizar a sua inscrição. Por favor, tente novamente mais tarde.');
			});
		});
...
function ShareContentToUserStatus( text, urlToShare, appID, posturl )
{
	...
	new Ajax.Request( posturl, {
		...
		onFailure: function(transport) {
			ShowAlertDialog( 'Compartilhar', 'Houve um problema ao compartilhar a atualização de status. Por favor, tente novamente mais tarde.' );
		}
	});
}
