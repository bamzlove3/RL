...
function checkAbuseSub( elForm )
{
	...
	{
		alert( 'Vyberte důvod hlášení zneužití' );
		return false;
	}
	...
	$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', params).done( function() {
		...
		ShowAlertDialog( 'Nahlásit', 'Při ukládání Vašeho nahlášení došlo k chybě. Zkuste to prosím později znovu.' );
	});
	return false;
}
...
function ChangeLanguage( strTargetLanguage, bStayOnPage )
{
	...
	$J.post( 'https://steamcommunity.com/actions/SetLanguage/', {language: strTargetLanguage, sessionid: g_sessionID })
		.done( function() {
			...
			Modal.Dismiss();
			ShowAlertDialog( 'Změnit jazyk', 'Při komunikaci se servery služby Steam došlo k chybě. Zkuste to prosím později znovu.' );
		});
}
...
var CCommentThread = Class.create( {
	...
	initialize: function( type, name, rgCommentData, url, nQuoteBoxHeight )
	{
		...
		this.m_$SubscribeCheckbox.click( function() {
			...
			if ( _this.m_bSubscribed )
				_this.Unsubscribe( function() {},  function() {ShowAlertDialog('Odebírat vlákno', 'Při aktualizaci Vašeho odběru došlo k chybě. Zkuste to prosím znovu později.');});
			else
				_this.Subscribe( function() {},  function() {ShowAlertDialog('Odebírat vlákno', 'Při aktualizaci Vašeho odběru došlo k chybě. Zkuste to prosím znovu později.');});
		});
	...
	{
		var strMessage = 'Omlouváme se, vyskytla se chyba: ';
		if ( transport.responseJSON && transport.responseJSON.error )
			strMessage += transport.responseJSON.error;
		else
			strMessage += 'Při komunikaci se sítí došlo k chybě. Zkuste to prosím později znovu.';
	...
	BindSubscribeButtons: function( elBtnSubscribe, elBtnUnsubscribe )
	{
		...
		$J(elBtnSubscribe).click( function() {
			_this.Subscribe( function() {
				...
				ShowAlertDialog('Odebírat vlákno', 'Při aktualizaci Vašeho odběru došlo k chybě. Zkuste to prosím znovu později.');
			});
		});
		...
		$J(elBtnUnsubscribe).click( function() {
			_this.Unsubscribe( function() {
				...
				ShowAlertDialog('Zrušit odběr vlákna', 'Při aktualizaci Vašeho odběru došlo k chybě. Zkuste to prosím znovu později.');
			});
		});
...
CCommentThread.FormattingHelpPopup = function( strCommentThreadType )
{
	$J.get( 'https://steamcommunity.com/comment/' + strCommentThreadType + '/formattinghelp', {ajax:1} ).done( function(data) {
		ShowAlertDialog( 'Úprava textu', data );
	});
};
...
function ShareContentToUserStatus( text, urlToShare, appID, posturl )
{
	...
	new Ajax.Request( posturl, {
		...
		onSuccess: function(transport) {
			...
			ShowAlertDialog( 'Sdílet', 'Status byl zveřejněn ve vaší Aktivitě přátel.' );
		},
		onFailure: function(transport) {
			ShowAlertDialog( 'Sdílet', 'Při sdílení statusu nastal problém. Zkuste to prosím znovu později.' );
		}
	});
}
