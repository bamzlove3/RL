	...
	initialize: function( type, name, rgCommentData, url, nQuoteBoxHeight )
	{
		...
		this.m_$SubscribeCheckbox.click( function() {
			...
			if ( _this.m_bSubscribed )
				_this.Unsubscribe( function() {},  function() {ShowAlertDialog('订阅留言帖', '更新您的订阅时出现问题。请稍后再试。');});
			else
				_this.Subscribe( function() {},  function() {ShowAlertDialog('订阅留言帖', '更新您的订阅时出现问题。请稍后再试。');});
		});
	...
	BindSubscribeButtons: function( elBtnSubscribe, elBtnUnsubscribe )
	{
		...
		$J(elBtnSubscribe).click( function() {
			_this.Subscribe( function() {
				ShowAlertDialog('订阅留言帖', '当有人回复该帖子时您将收到留言通知。');
			}, function() {
				ShowAlertDialog('订阅留言帖', '更新您的订阅时出现问题。请稍后再试。');
			});
		});

		$J(elBtnUnsubscribe).click( function() {
			_this.Unsubscribe( function() {
				ShowAlertDialog('取消订阅留言帖', '您将不再收到该帖子的留言通知。');
			}, function() {
				ShowAlertDialog('取消订阅留言帖', '更新您的订阅时出现问题。请稍后再试。');
			});
		});
