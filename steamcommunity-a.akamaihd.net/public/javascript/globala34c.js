...
function checkAbuseSub( elForm )
{
	...
	$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', params).done( function() {
		ShowAlertDialog( '¡Gracias!', 'Gracias por informar de la existencia de contenido ofensivo y ayudarnos a mantener un ambiente cordial y limpio en la Comunidad Steam.' );
	}).fail( function() {
		ShowAlertDialog( 'Informar de infracción', 'Ha habido un problema al guardar tu denuncia. Por favor, inténtalo de nuevo más tarde.' );
	});
	return false;
}
...
function AlertNonSteamSite( elem )
{
	...
	if ( hostname )
	{
		...
		return confirm( 'Nota: el URL sobre el que has hecho clic no es el de una web oficial de Steam.\n\n'
						...
						+ '¿Estás seguro de que quieres visitar esta página? Haz clic en Aceptar para continuar bajo tu propia responsabilidad.\n' );
	}
...
function ShowAbuseDialog()
{
	...
	if ( g_AbuseModalContents )
	{
		var Modal = ShowDialog( 'Informar de infracción', g_AbuseModalContents );
	}
}
