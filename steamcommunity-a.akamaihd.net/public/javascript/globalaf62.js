...
function checkAbuseSub( elForm )
{
	...
	$J.post( 'https://steamcommunity.com/actions/ReportAbuse/', params).done( function() {
		ShowAlertDialog( '感謝您！', '感謝您回報不當內容，協助 Steam 社群維持乾淨友善的空間。' );
	}).fail( function() {
		ShowAlertDialog( '回報違規事項', '存取您的回報時出現問題。請稍號再試。' );
	});
	return false;
}
...
function AlertNonSteamSite( elem )
{
	...
	if ( hostname )
	{
		...
		return confirm( '附註：您點擊的網址並非 Steam 官方網站。\n\n'
						+ url.replace( new RegExp( '^steam://openurl(_external)?/' ), '' ) + '\n\n'
						+ '如果該網站向您詢問您的使用者名稱和密碼，絕對不要輸入它們。您有可能會失去您的 Steam 帳戶和所有的遊戲！\n'
						+ '您確定要瀏覽此頁面？點擊「確定」以繼續並請您自己承擔風險。\n' );
	}
...
function ChangeLanguage( strTargetLanguage, bStayOnPage )
{
	...
	$J.post( 'https://steamcommunity.com/actions/SetLanguage/', {language: strTargetLanguage, sessionid: g_sessionID })
		.done( function() {
			...
			Modal.Dismiss();
			ShowAlertDialog( '變更語言', '無法正常與 Steam 伺服器連線。請稍候再試。' );
		});
}
	...
	initialize: function( type, name, rgCommentData, url, nQuoteBoxHeight )
	{
		...
		this.m_$SubscribeCheckbox.click( function() {
			...
			if ( _this.m_bSubscribed )
				_this.Unsubscribe( function() {},  function() {ShowAlertDialog('訂閱文章', '更新您的訂閱時發生錯誤。請稍候再試。');});
			else
				_this.Subscribe( function() {},  function() {ShowAlertDialog('訂閱文章', '更新您的訂閱時發生錯誤。請稍候再試。');});
		});
	...
	BindSubscribeButtons: function( elBtnSubscribe, elBtnUnsubscribe )
	{
		...
		$J(elBtnSubscribe).click( function() {
			_this.Subscribe( function() {
				ShowAlertDialog('訂閱文章', '當有人回覆此文章時，您將會收到通知訊息。');
			}, function() {
				ShowAlertDialog('訂閱文章', '更新您的訂閱時發生錯誤。請稍候再試。');
			});
		});

		$J(elBtnUnsubscribe).click( function() {
			_this.Unsubscribe( function() {
				ShowAlertDialog('取消文章訂閱', '您將不會再收到關於此文章的通知訊息。');
			}, function() {
				ShowAlertDialog('取消文章訂閱', '更新您的訂閱時發生錯誤。請稍候再試。');
			});
		});
...
function ShareContentToUserStatus( text, urlToShare, appID, posturl )
{
	...
	new Ajax.Request( posturl, {
		...
		onSuccess: function(transport) {
			...
			ShowAlertDialog( '分享', '該留言已經張貼於您的好友動態。' );
		},
		onFailure: function(transport) {
			ShowAlertDialog( '分享', '分享留言時發生問題，請稍後再試一次。' );
		}
	});
}
